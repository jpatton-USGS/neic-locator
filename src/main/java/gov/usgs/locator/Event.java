...
public class Event {
   ...
   * an analyst.
   */
  private boolean isDepthPreferred;    
  ...
  /**
   * An int containing the number of stations associated with this event.
   */
  private int numStationsAssociated;

  /** 
   * An int containing the number of stations used in this event.
   */
  ...
  public void input(LocInput in) {
    ...
    isDepthPreferred = in.getIsBayesianDepth();
    ...
    // Take care of some other event initialization.
    initEvent();
  }
  ...
  public LocOutput output() {
    ...
    out.addErrors(timeStandardError, latitudeStandardError, 
        ...
        hypo.bayesianDepth, hypo.bayesianDepthSpread, 
  ...
  private void initEvent() {
    ...
    if (isDepthPreferred) {
      if (bayesianDepthSpread > 0d) {
        ...
        isDepthPreferred = false;    // Trap a bad command
      }
    }

    // If we're decorrelating, instantiate some more objects.
    if (useDecorrelation) {
    ...
    // Initialize hasPhaseIdChanged and the depth importance.
...
=======
  ...
  public void printHydraInput() {
    System.out.format("\n%22s %8.4f %9.4f %6.2f %5b %5b %5b "
        + "%5.1f %5.1f %5b\n", LocUtil.getRayDate(hypo.originTime), 
        hypo.latitude, hypo.longitude, hypo.depth, isLocationHeld, isDepthHeld, 
        isDepthPreferred, hypo.bayesianDepth, hypo.bayesianDepthSpread, 
        useDecorrelation);
    System.out.println();
  ...
  public void printHydraOutput() {
    ...
    System.out.format("%6.2f %6.1f %6.1f %6.1f %6.2f %6.1f %6.1f %6.1f "
        ...
        hypo.bayesianDepth, hypo.bayesianDepthSpread, bayesianDepthDataImportance);
 ...
 /**
   * Function to return the number of stations used by the event
   * 
   * @return A int containing the number of stations used by the event
   */
  ...
  /**
   * Function to set the maximum horizontal projection of the error ellipsoid in 
   * kilometers.
   * 
   * @param maxHorizontalError A double containing the maximum horizontal 
   *                           projection of the error ellipsoid in kilometers.
   */
  void setMaxHorizontalError(double maxHorizontalError) {
    this.maxHorizontalError = maxHorizontalError;
  }

  /**
   * Function to set the maximum vertical projection of the error ellipsoid in 
   * kilometers.
   * 
   * @param maxVerticalError A double containing the maximum vertical projection 
   *                         of the error ellipsoid in kilometers.
   */
  void setMaxVerticalError(double maxVerticalError) {
    this.maxVerticalError = maxVerticalError;
  }
   ...
   * analyst.
   */
  public boolean getIsDepthPreferred() {
    return isDepthPreferred;
  }
...
>>>>>>> further code standard changes, locator output changed slightly
}
